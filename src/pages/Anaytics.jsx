...
function MiniBar({ value, max, color = "indigo" }) {
    ...
    const map = { indigo: "bg-indigo-500", blue: "bg-blue-500", emerald: "bg-emerald-500", purple: "bg-purple-500", orange: "bg-orange-500", rose: "bg-rose-500", gray: "bg-gray-300" };
    return (
        <div className="w-full bg-gray-100 rounded-full h-1.5 mt-1.5">
            <div className={`h-1.5 rounded-full ${map[color] || "bg-indigo-500"} transition-all duration-500`} style={{ width: `${pct}%` }} />
        </div>
    );
}
    ...
    return (
        <div className="flex flex-col items-center justify-center py-8 gap-2 text-center">
            <div className="w-9 h-9 bg-gray-100 rounded-full flex items-center justify-center">
                <i data-lucide={icon} className="w-4 h-4 text-gray-400" />
            </div>
    ...
    return (
        <div className={`bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden ${className}`}>
            <div className="px-5 py-3.5 border-b border-gray-100 flex items-center gap-2">
                <i data-lucide={icon} className="w-4 h-4 text-indigo-500 flex-shrink-0" />
...
function StatCard({ icon, label, value, sub, color = "indigo", change }) {
    const map = {
    ...
    return (
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 flex items-start gap-3">
            <div className={`w-9 h-9 ${c.bg} ring-1 ${c.ring} rounded-lg flex items-center justify-center flex-shrink-0`}>
                <i data-lucide={icon} className={`w-4 h-4 ${c.ic}`} />
            </div>
...
function RangeButton({ active, onClick, label }) {
    return (
        <button onClick={onClick} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${active ? "bg-indigo-600 text-white" : "text-gray-500 hover:text-gray-700 hover:bg-gray-100"}`}>
            {label}
        </button>
    );
}

function BarChart({ data, keyX, keyY, color = "bg-indigo-500", height = "h-36" }) {
    const max = Math.max(...data.map((d) => Number(d[keyY]) || 0), 1);
    return (
        <div className={`flex items-end gap-0.5 ${height} w-full`}>
            {data.map((d, i) => {
                const pct = Math.max((Number(d[keyY]) / max) * 100, 1);
                const label = d[keyX] ? new Date(d[keyX]).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "";
                return (
                    <div key={i} className="flex flex-col items-center flex-1 min-w-0 group relative">
                        <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-[10px] px-1.5 py-0.5 rounded-sm opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-10">
                            {label}: {fmt(d[keyY])}
                        </div>
                        <div className={`w-full ${color} rounded-t-sm hover:opacity-80 transition-opacity cursor-default`} style={{ height: `${pct}%` }} />
                    </div>
                );
            })}
        </div>
...
function TwoLineChart({ data }) {
    if (!data?.length) return <EmptyState icon="trending-up" msg="No data for this period." />;
    const max = Math.max(...data.map((d) => Math.max(Number(d.total_views) || 0, Number(d.unique_visitors) || 0)), 1);
    return (
        <div className="relative h-36 w-full">
            <svg className="w-full h-full" viewBox={`0 0 ${data.length * 10} 100`} preserveAspectRatio="none">
                <polyline
                    fill="none" stroke="#6366f1" strokeWidth="1.5"
                    points={data.map((d, i) => `${i * 10 + 5},${100 - (Number(d.total_views) / max) * 90}`).join(" ")}
                />
                <polyline
                    fill="none" stroke="#10b981" strokeWidth="1.5" strokeDasharray="3,2"
                    points={data.map((d, i) => `${i * 10 + 5},${100 - (Number(d.unique_visitors) / max) * 90}`).join(" ")}
                />
            </svg>
            <div className="flex items-center gap-4 mt-2">
                <span className="flex items-center gap-1.5 text-xs text-gray-500"><span className="w-3 h-0.5 bg-indigo-500 inline-block" /> Page views</span>
                <span className="flex items-center gap-1.5 text-xs text-gray-500"><span className="w-3 h-0.5 bg-emerald-500 inline-block" style={{ borderTop: "1.5px dashed #10b981", display: "inline-block" }} /> Unique visitors</span>
            </div>
        </div>
    );
}
...
function DonutRing({ data, total }) {
    if (!data?.length || !total) return <EmptyState icon="pie-chart" msg="No data." />;
        ...
        <div className="flex items-center gap-4">
            <svg viewBox="0 0 100 100" className="w-24 h-24 flex-shrink-0 -rotate-90">
                {data.map((d, i) => {
                    ...
                    const el = <circle key={i} cx="50" cy="50" r={r} fill="none" stroke={colors[i % colors.length]} strokeWidth="18" strokeDasharray={`${stroke} ${circ - stroke}`} strokeDashoffset={-offset * circ / 1} style={{ strokeDashoffset: -(offset * circ) }} />;
                    offset += pct;
                    return el;
                })}
            </svg>
...
export default function Analytics() {
    ...
    useEffect(() => { load(range); }, [selectedWebsite?.id, range]);
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3 text-center">
                <div className="w-14 h-14 bg-indigo-50 rounded-full flex items-center justify-center">
                    <i data-lucide="bar-chart-2" className="w-7 h-7 text-indigo-400" />
                </div>
    ...
    const maxCountry = d.countryBreakdown?.[0]?.views || 1;
    ...
    const skRow = (n = 5) => <div className="space-y-3">{Array.from({ length: n }).map((_, i) => <Skeleton key={i} className="h-9 rounded" />)}</div>;
        ...
        <>
            <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
                    {RANGES.map((r) => <RangeButton key={r.value} label={r.label} active={range === r.value} onClick={() => { setRange(r.value); load(r.value); }} />)}
                </div>
            </div>

            {error && (
                <div className="mb-4 flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 rounded-lg">
                    <i data-lucide="alert-circle" className="w-4 h-4 flex-shrink-0" />
                    {error}
                    <button onClick={clearError} className="ml-auto"><i data-lucide="x" className="w-4 h-4" /></button>
                </div>
            )}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                {loading ? Array.from({ length: 8 }).map((_, i) => <Skeleton key={i} className="h-20 rounded-lg" />) : <>
                    <StatCard icon="eye" label="Page Views" value={fmt(s.total_views)} change={delta(s.total_views, p.total_views)} color="blue" />
                    <StatCard icon="users" label="Unique Visitors" value={fmt(s.unique_visitors)} change={delta(s.unique_visitors, p.unique_visitors)} color="indigo" />
                    <StatCard icon="activity" label="Sessions" value={fmt(s.sessions)} change={delta(s.sessions, p.sessions)} color="purple" />
                    <StatCard icon="percent" label="Bounce Rate" value={fmtPct(s.bounce_rate)} change={s.bounce_rate != null && p.bounce_rate != null ? -delta(s.bounce_rate, p.bounce_rate) : null} color="orange" />
                    <StatCard icon="clock" label="Avg Session" value={fmtDur(s.avg_duration)} sub={`${fmtPct(100 - (s.bounce_rate || 0))} engaged`} color="emerald" />
                    <StatCard icon="layers" label="Pages / Session" value={s.avg_pages ? Number(s.avg_pages).toFixed(1) : "—"} color="blue" />
                    <StatCard icon="zap" label="Total Events" value={fmt(s.total_events)} color="purple" />
                    <StatCard icon="file-check" label="Form Completions" value={fmt(s.form_completions)} sub={s.form_starts ? `of ${fmt(s.form_starts)} starts` : undefined} color="emerald" />
                </>}
            </div>

            {/* ── Traffic Over Time ── */}
            <div className="mb-6">
                <Card title="Traffic Over Time" icon="trending-up">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                <Card title="New vs Returning" icon="users">
                ...
                <div className="lg:col-span-2">
                    <Card title="Daily Breakdown" icon="calendar">
                        {loading ? <Skeleton className="h-36 rounded" /> : !d.dailyStats?.length ? (
                            <EmptyState icon="calendar" msg="No daily stats yet." />
                        ) : (
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {d.dailyStats.slice(-14).reverse().map((row, i) => (
                                            <tr key={i} className="hover:bg-gray-50 transition-colors">
                                                <td className="py-1.5 pr-4 text-gray-500 whitespace-nowrap">{new Date(row.stat_date).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                <div className="lg:col-span-2">
                    <Card title="Top Pages" icon="file-text">
                        {loading ? skRow() : !d.topPages?.length ? <EmptyState icon="file" msg="No page data." /> : (
                            <div className="space-y-3">
                                {d.topPages.map((p, i) => (
                                    <div key={i}>
                ...
                <div className="space-y-4">
                    <Card title="Entry Pages" icon="log-in">
                        {loading ? skRow(4) : !d.topEntryPages?.length ? <EmptyState icon="log-in" msg="No entry data." /> : (
                            <div className="space-y-3">
                    ...
                    <Card title="Exit Pages" icon="log-out">
                        {loading ? skRow(4) : !d.topExitPages?.length ? <EmptyState icon="log-out" msg="No exit data." /> : (
                            <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                <Card title="Devices" icon="monitor">
                    {loading ? <Skeleton className="h-32 rounded" /> : !d.deviceBreakdown?.length ? <EmptyState icon="monitor" msg="No device data." /> : (
                        <DonutRing data={d.deviceBreakdown} total={totalDevices} />
                    )}
                </Card>

                <Card title="Browsers" icon="globe">
                    {loading ? skRow(4) : !d.browserBreakdown?.length ? <EmptyState icon="globe" msg="No browser data." /> : (
                        <div className="space-y-3">
                ...
                <Card title="Operating Systems" icon="cpu">
                    {loading ? skRow(4) : !d.osBreakdown?.length ? <EmptyState icon="cpu" msg="No OS data." /> : (
                        <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                <Card title="Top Referrers" icon="link">
                    {loading ? skRow() : !d.topReferrers?.length ? <EmptyState icon="link-2" msg="No referrer data." /> : (
                        <div className="space-y-3">
                ...
                <Card title="Countries" icon="globe-2">
                    {loading ? skRow() : !d.geoStats?.length ? <EmptyState icon="map-pin" msg="No geo data." /> : (
                        <div className="space-y-3">
            ...
            {/* ── Events ── */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                <Card title="Top Events" icon="zap">
                    {loading ? skRow() : !d.topEvents?.length ? <EmptyState icon="zap" msg="No events tracked yet." /> : (
                        <div className="space-y-3">
                            {d.topEvents.map((e, i) => (
                                <div key={i}>
                                    <div className="flex items-center justify-between text-xs">
                                        <div className="flex items-center gap-1.5 min-w-0">
                                            ...
                                            {e.event_category && <span className="bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded text-[10px] flex-shrink-0">{e.event_category}</span>}
                ...
                <Card title="Events Over Time" icon="activity">
                    {loading ? <Skeleton className="h-36 rounded" /> : !d.eventTimeSeries?.length ? <EmptyState icon="activity" msg="No event data." /> : (
                        <BarChart data={d.eventTimeSeries} keyX="date" keyY="count" color="bg-orange-400" />
                    )}
                </Card>
            </div>
            ...
            {/* ── Performance ── */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                <Card title="Performance Overview" icon="gauge">
                    {loading ? <div className="grid grid-cols-2 gap-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-16 rounded" />)}</div> :
                        !perf.avg_load_time && !perf.avg_fcp ? <EmptyState icon="gauge" msg="No performance data yet." /> : (
                            <div className="grid grid-cols-2 gap-3">
                                {[
                                    { label: "Avg Load Time", value: fmtMs(perf.avg_load_time), min: fmtMs(perf.min_load_time), max: fmtMs(perf.max_load_time), icon: "clock", good: perf.avg_load_time < 3000 },
                                    { label: "First Paint", value: fmtMs(perf.avg_fp), icon: "paintbrush-2", good: perf.avg_fp < 1000 },
                                    { label: "FCP", value: fmtMs(perf.avg_fcp), icon: "image", good: perf.avg_fcp < 1800 },
                                    { label: "TTI", value: fmtMs(perf.avg_tti), icon: "mouse-pointer", good: perf.avg_tti < 3800 },
                                    { label: "DOM Interactive", value: fmtMs(perf.avg_dom_interactive), icon: "code-2", good: perf.avg_dom_interactive < 2000 },
                                    { label: "Samples", value: fmt(perf.sample_count), icon: "database", good: true },
                                ].map((m, i) => (
                                    <div key={i} className="rounded-lg border border-gray-100 p-3">
                                        <div className="flex items-center gap-1 mb-1">
                                            <i data-lucide={m.icon} className="w-3 h-3 text-gray-400" />
                                            <p className="text-[11px] text-gray-400">{m.label}</p>
                                        </div>
                                        <span className={`text-base font-semibold px-1.5 py-0.5 rounded ${m.good ? "text-emerald-700 bg-emerald-50" : "text-rose-700 bg-rose-50"}`}>{m.value}</span>
                ...
                <Card title="Slowest Pages" icon="timer">
                    {loading ? skRow() : !d.perfByPage?.length ? <EmptyState icon="timer" msg="No page performance data." /> : (
                        <div className="space-y-3">
                                ...
                                <div key={i}>
                                    <div className="flex justify-between text-xs">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <span className={`font-semibold ${Number(p.avg_load_time) > 3000 ? "text-rose-600" : "text-gray-900"}`}>{fmtMs(p.avg_load_time)}</span>
                                        </div>
                                    </div>
                                    <MiniBar value={Number(p.avg_load_time)} max={maxPerfPage} color={Number(p.avg_load_time) > 3000 ? "rose" : "emerald"} />
                                </div>
                            ))}
                        </div>
                    )}
            ...
            {(loading || d.formStats?.length > 0) && (
                <div className="mb-6">
                    <Card title="Form Analytics" icon="clipboard-list">
                        {loading ? skRow(3) : !d.formStats?.length ? <EmptyState icon="clipboard" msg="No form data." /> : (
                            <div className="overflow-x-auto">
                                <table className="w-full text-xs text-left">
                                    <thead>
                                        <tr className="text-gray-400 border-b border-gray-100">
                                        ...
                                        {d.formStats.map((f, i) => (
                                            <tr key={i} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="py-2 pr-6">
                                                    <span className={`px-1.5 py-0.5 rounded font-medium ${Number(f.completion_rate) >= 50 ? "bg-emerald-50 text-emerald-700" : "bg-orange-50 text-orange-700"}`}>
